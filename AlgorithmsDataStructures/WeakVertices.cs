...
namespace AlgorithmsDataStructures2
{
    ...
    public class SimpleGraph<T>
    {
        ...
        public List<Vertex<T>> WeakVertices()
        {
            ...
            int?[] neighbourVertices = new int?[m_adjacency.GetUpperBound(0)];
            int index = 0;
            List<Vertex<T>> result = new List<Vertex<T>>();
            for (int i = 0; i <= m_adjacency.GetUpperBound(0); i++)
            {
                for (int j = 0; j <= m_adjacency.GetUpperBound(0); j++)
                {
                    if (m_adjacency[i, j] == 1)
                    {
                        neighbourVertices[index] = j;
                        index++;
                    }
                }
        ...
        public bool IsLinked (int?[] a)
        {
            bool isLinked = false;
            int neighbourVerticesSize = GetArraySize(a);
            for (int l = 0; l < neighbourVerticesSize - 1; l++)
            {
                for (int k = l + 1; k < neighbourVerticesSize; k++)
                {
                    if (IsEdge((int)a[l], (int)a[k])) isLinked = true;
                }
            }
            return isLinked;
        }
